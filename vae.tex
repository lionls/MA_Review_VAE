...
\subsection{BiMeanVAE}
BiMeanVAE ist ein von \citep{coop} vorgestelltes Model bestehend aus einem bidirektionalem LSTM Encoder und einem LSTM Decoder. 
...
BiMeanVAE verwendet die Standard Variational Autoencoder Ziele, dass minimieren des Rekonstruktionsfehlers mit KL-Regularisierung verwendet.
...
Im Latentspace werden Eingabesequenzen von Optimus und BiMeanVAE jeweils durch einen Latentvektor dargestellt.
...
So lässt sich aus einer Untergruppe der Vektoren $\mathcal{Z}^{'} \subseteq \mathcal{Z}$ durch geschicktes kombinieren eine Durchschnittsrepräsentation im Latentvektorraum finden.
